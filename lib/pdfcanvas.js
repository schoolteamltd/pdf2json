...
let nodeUtil = require("util"),
  ...
  PDFUnit = require('./pdfunit.js'); /* Fixes */
...
console.log('-- FillText');console.log('x: ' + x + ', y: ' + y);console.log(this.m_);
    ...
    contextPrototype.drawImage = function (image, x, y, width, height) {console.log('drawImage');
    x = x || 0;
    y = y || 0;
    width = width || 0;
    height = height || 0;
      ...
      let p = this.getCoords_(x, y);
      this.canvas.Images.push({
        x: PDFUnit.toFormX(p.x),
        y: PDFUnit.toFormY(p.y),
        width: PDFUnit.toFormX(width),
        height: PDFUnit.toFormY(height),
        obj: image
      });
    };

    contextPrototype.getImageData = function (x, y, w, h) {console.log('getImageData');console.log(args);
